         ...
         <div class="col-md-7">
          <div class="card" style="padding: 2%">
           ...
           @endif
            ...
            @if ( $product->pharm_standard === Null)
            <textarea  name="pharm_standard" class="textarea" style="font-size: 16px" cols="30" rows="6"><p style="text-align: justify;"><span style="font-size: 12pt;"> {{\App\PharmStandards::find($pharmreports->productType->pharm_standard_id)? \App\PharmStandards::find($pharmreports->productType->pharm_standard_id)->default:'' }} </span></p></textarea>
            @endif

               <h4 class="font" style="font-size:18px margin:20px; margin-top:15px"> <strong>RESULT: </strong></h4>
               @if ($pharmreports->pharm_result ===Null)
                 <textarea class="textarea" style="font-size: 16px"name="pharm_result" rows="10"> <p style="text-align: justify;"><span style="font-size: 12pt;"> The experimental @foreach ($pharmreports->animalExperiment->unique('animal_model') as $item){{App\PharmAnimalModel::find($item->animal_model)->name}} @endforeach in group 1 and 2 administered with 1% and 5% w/v of the {{$pharmreports->productType->name}} intradermally and topically (0.5-1 g), showed no ulceration, irritation and/or inflammations at the site of injection and shaved area, respectively.</span></p></textarea>
                 @endif
                 @if ($pharmreports->pharm_result !== Null)
                <textarea style="font-size: 16px" class="textarea" rows="10" name="pharm_result" >{{$pharmreports->pharm_result}}
                </textarea>
                @endif
                ...
                <textarea  style="font-size: 16px text-align: justify " class="textarea" rows="4" name="pharm_dermal_comment" >{!! $pharmreports->pharm_dermal_comment !!}
                </textarea>
                @endif
                @if ( $product->pharm_dermal_comment === Null)
                <textarea  style="font-size: 16px text-align: justify " class="textarea" rows="4" name="pharm_dermal_comment" ><span style="font-size: 12pt;"> {{$pharmreports->code}} appears to be safe when applied to the skin.</span></textarea>
                @endif
